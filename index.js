...
async function sendMail(email) 
{
  ...
  var mailInfo = await transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      console.log(error);
    } else {
...
app.post("/checkotp", encoder ,(req, res) => {
...
app.listen(port, () => {
  console.log(`file is run on port ${port} http://localhost:5000/`)
});
